...
class FollowMeWebMedicalController extends Controller
{
    ...
    public function medical_patient_select(Request $request){
        $patient = testPatient::find($request->input('patient_id'));     //환자 데이터
        $clinic  = $patient->clinic()->where('standby_status' , '1')     //환자의 현 진료 데이터
                            ->orderBy("clinic_time", "desc")->first();   
        $flows    = testPatient::find($request->input('patient_id'))     //환자의 현 동선 데이터
    ...
    public function medical_clinic_end(Request $request){
        //환자 전체 대기순번 -1씩
        testClinic::where("standby_status", 1)->decrement("standby_number");
        
        //진료가 완료된 환자 정보 처리
        $clinic = testPatient::find($request->input('patient_id'))->clinic()->where('standby_status', 1)
